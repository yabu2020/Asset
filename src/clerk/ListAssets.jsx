...
function ListAssets() {
  ...
  const saveChanges = (assetId) => {
    axios.put(`http://localhost:3001/updateasset/${assetId}`, editData)
      .then(response => {
        setAssets(assets.map(category => ({
          ...category,
          assets: category.assets.map(asset => asset._id === assetId ? response.data : asset)
        })));
        ...
        <div className="overflow-x-auto">
          {assets.length > 0 ? assets.map((categoryGroup, index) => (
            <div key={index} className="mb-6">
              <h2 className="text-2xl font-bold mb-4">{categoryGroup._id}</h2>
              <table className="w-full flex-grow min-w-[540px]">
                ...
                <tbody>
                  {categoryGroup.assets.length > 0 ? categoryGroup.assets.map((asset) => (
                    ...
                    <tr>
                      <td colSpan="8">No assets found in this category</td>
                    </tr>
                  )}
                </tbody>
              </table>
